...
public class RunnerRating extends BaseContentPanel {

    private RunnerModel runner;
    private Map<Integer, List<OrderModel>> orderMap;
    private String selectedRating;

    public RunnerRating(RunnerModel runnerModel) {
        super();
        this.runner = UserServices.findRunnerById(runnerModel.getId());
        this.orderMap = RunnerOrderServices.getRunnerOrdersGroupByRating(runner.getId());
    ...
    private Node header() {
        return new TitleBackButton("Review - " + runner.getName(), () -> {
            ManagerViewModel.getRunnerPerformanceViewModel()
                    .navigate(ManagerViewModel.getRunnerPerformanceViewModel().getRunnerListUI());
        });
    }

    private Node content() {
        Double rating = RunnerOrderServices.getRunnerOverallRating(runner.getId());
        Label overallRating = new Label("Overall Rating: " +
                (rating == null || Double.isNaN(rating) ? "No rating available" : String.format("%.2f", rating)));
